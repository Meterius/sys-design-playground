use crate::glam_ext::bounding::{AxisAlignedBoundingBox2D, DAabb2};
use anyhow::anyhow;
use glam::{DVec2, dvec2};
use image::{Rgb, RgbImage};
use irox_carto::coordinate::EllipticalCoordinate;
use irox_carto::proj::Projection;
use irox_carto::tm::TransverseMercator;
use itertools::Itertools;
use serde::Deserialize;
use std::simd::num::SimdFloat;
use std::simd::{Simd, StdFloat};

#[derive(Debug, Deserialize)]
#[serde(rename_all = "PascalCase")]
pub struct UserData {
    pub name: String,
    pub geo_footprint: GeoFootprint,
}

#[derive(Debug, Deserialize)]
pub struct GeoFootprint {
    pub coordinates: Vec<Vec<[f64; 2]>>,
}

fn sample_bilinear(img: &RgbImage, p: DVec2) -> Rgb<u8> {
    let w = img.width() as i32;
    let h = img.height() as i32;

    let x = p.x;
    let y = p.y;

    let x0 = x.floor() as i32;
    let y0 = y.floor() as i32;
    let x1 = x0 + 1;
    let y1 = y0 + 1;

    if x0 < 0 || y0 < 0 || x1 >= w || y1 >= h {
        return Rgb([0, 0, 0]);
    }

    let fx = x - x0 as f64;
    let fy = y - y0 as f64;

    let c00 = img.get_pixel(x0 as u32, y0 as u32).0;
    let c10 = img.get_pixel(x1 as u32, y0 as u32).0;
    let c01 = img.get_pixel(x0 as u32, y1 as u32).0;
    let c11 = img.get_pixel(x1 as u32, y1 as u32).0;

    let mut out = [0u8; 3];

    for i in 0..3 {
        let v = (1.0 - fx) * (1.0 - fy) * c00[i] as f64
            + fx * (1.0 - fy) * c10[i] as f64
            + (1.0 - fx) * fy * c01[i] as f64
            + fx * fy * c11[i] as f64;

        out[i] = v.clamp(0.0, 255.0) as u8;
    }

    Rgb(out)
}

/// Extract bounds from a cardinal-offset directory name, e.g. `N18E000`.
/// Each tile covers a 36° × 36° cell.
pub fn extract_bounds_offset(dir_name: &str) -> anyhow::Result<DAabb2> {
    let re = regex::Regex::new(r"([NS])(\d+)([EW])(\d+)")?;
    let caps = re
        .captures(dir_name)
        .ok_or_else(|| anyhow!("No cardinal-offset pattern found in {dir_name:?}"))?;

    let lat_val: f64 = caps[2].parse()?;
    let lon_val: f64 = caps[4].parse()?;

    let min_lat = if &caps[1] == "S" { -lat_val } else { lat_val };
    let min_lon = if &caps[3] == "W" { -lon_val } else { lon_val };

    Ok(DAabb2::new(
        dvec2(min_lon, min_lat),
        dvec2(min_lon + 36.0, min_lat + 36.0),
    ))
}

pub fn convert_sen2_img_to_epsg4326(
    img: &RgbImage,
    data: &UserData,
) -> anyhow::Result<(RgbImage, DAabb2)> {
    struct Bounds {
        top_left: DVec2,
        top_right: DVec2,
        bottom_right: DVec2,
        bottom_left: DVec2,
    }

    let bounds = Bounds {
        bottom_left: DVec2::from_array(data.geo_footprint.coordinates[0][0]),
        top_left: DVec2::from_array(data.geo_footprint.coordinates[0][1]),
        top_right: DVec2::from_array(data.geo_footprint.coordinates[0][2]),
        bottom_right: DVec2::from_array(data.geo_footprint.coordinates[0][3]),
    };

    let center = bounds.bottom_left;

    let utm_proj = TransverseMercator::builder()
        .with_center(EllipticalCoordinate::new_degrees_wgs84(center.y, center.x))
        .build();

    let utm_top_left = utm_proj.project_to_cartesian(&EllipticalCoordinate::new_degrees_wgs84(
        bounds.top_left.y,
        bounds.top_left.x,
    ));
    let utm_top_right = utm_proj.project_to_cartesian(&EllipticalCoordinate::new_degrees_wgs84(
        bounds.top_right.y,
        bounds.top_right.x,
    ));
    let utm_bottom_right = utm_proj.project_to_cartesian(&EllipticalCoordinate::new_degrees_wgs84(
        bounds.bottom_right.y,
        bounds.bottom_right.x,
    ));
    let utm_bottom_left = utm_proj.project_to_cartesian(&EllipticalCoordinate::new_degrees_wgs84(
        bounds.bottom_left.y,
        bounds.bottom_left.x,
    ));

    let utm_aabb = DAabb2::new(
        dvec2(
            (utm_bottom_left.get_x().value() + utm_top_left.get_x().value()) / 2.0,
            (utm_bottom_left.get_y().value() + utm_bottom_right.get_y().value()) / 2.0,
        ),
        dvec2(
            (utm_bottom_right.get_x().value() + utm_top_right.get_x().value()) / 2.0,
            (utm_top_right.get_y().value() + utm_top_right.get_y().value()) / 2.0,
        ),
    );

    let rect_bounds = DAabb2::new(
        dvec2(
            bounds.top_left.x.max(bounds.bottom_left.x),
            bounds.bottom_left.y.max(bounds.bottom_right.y),
        ),
        dvec2(
            bounds.top_right.x.min(bounds.bottom_right.x),
            bounds.top_right.y.min(bounds.top_left.y),
        ),
    );

    let out_w = img.width();
    let out_h = img.height();

    let mut out = RgbImage::new(out_w, out_h);
    let src_size = dvec2(img.width() as f64, img.height() as f64);
    let out_size = dvec2(out_w as f64, out_h as f64);

    fn src_pos_batched<const N: usize>(
        xs: Simd<f64, N>,
        ys: Simd<f64, N>,
        rect_bounds_min_rads_w: Simd<f64, N>,
        rect_bounds_min_rads_h: Simd<f64, N>,
        rect_bounds_size_rads_w: Simd<f64, N>,
        rect_bounds_size_rads_h: Simd<f64, N>,
        src_size_s1_w: Simd<f64, N>,
        src_size_s1_h: Simd<f64, N>,
        out_size_s1_w: Simd<f64, N>,
        out_size_s1_h: Simd<f64, N>,
        utm_proj: &TransverseMercator,
        utm_aabb: &DAabb2,
    ) -> (Simd<f64, N>, Simd<f64, N>) {
        let one = Simd::splat(1.0);
        let zero = Simd::splat(0.0);

        let rel_x = xs / out_size_s1_w;
        let rel_y = one - ys / out_size_s1_h;

        let lon_rads = rect_bounds_min_rads_w
            + rel_x * rect_bounds_size_rads_w;
        let lat_rads = rect_bounds_min_rads_h
            + rel_y * rect_bounds_size_rads_h;

        let (el_x, el_y) = utm_proj.project_to_cartesian_batch(lon_rads, lat_rads);

        let src_x_rel = (el_x - Simd::splat(utm_aabb.min().x)) / Simd::splat(utm_aabb.size().x);
        let src_y_rel = one
            - (el_y - Simd::splat(utm_aabb.min().y)) / Simd::splat(utm_aabb.size().y);

        let src_x = src_x_rel.simd_clamp(zero, one)
            * src_size_s1_w;
        let src_y = src_y_rel.simd_clamp(zero, one)
            * src_size_s1_h;

        (src_x.round(), src_y.round())
    }

    let out_size_s1_w = Simd::splat(out_size.x - 1.0);
    let out_size_s1_h = Simd::splat(out_size.y - 1.0);

    let out_size_s1_w_1 = Simd::splat(out_size.x - 1.0);
    let out_size_s1_h_1 = Simd::splat(out_size.y - 1.0);

    let src_size_s1_w = Simd::splat(src_size.x - 1.0);
    let src_size_s1_h = Simd::splat(src_size.y - 1.0);

    let src_size_s1_w_1 = Simd::splat(src_size.x - 1.0);
    let src_size_s1_h_1 = Simd::splat(src_size.y - 1.0);

    let rect_bounds_min_rads_w = Simd::splat(rect_bounds.min().x.to_radians());
    let rect_bounds_min_rads_h = Simd::splat(rect_bounds.min().y.to_radians());
    let rect_bounds_size_rads_w = Simd::splat(rect_bounds.size().x.to_radians());
    let rect_bounds_size_rads_h = Simd::splat(rect_bounds.size().y.to_radians());

    let rect_bounds_min_rads_w_1 = Simd::splat(rect_bounds.min().x.to_radians());
    let rect_bounds_min_rads_h_1 = Simd::splat(rect_bounds.min().y.to_radians());
    let rect_bounds_size_rads_w_1 = Simd::splat(rect_bounds.size().x.to_radians());
    let rect_bounds_size_rads_h_1 = Simd::splat(rect_bounds.size().y.to_radians());

    for y in 0..out_h {
        const N: usize = 64;

        for xs in (0..out_w).map(|x| x as f64).chunks(N).into_iter() {
            let xs = xs.collect_vec();
            if let Some(&xs) = xs.as_slice().as_array() {
                let (src_x, src_y) = src_pos_batched::<N>(
                    Simd::from_array(xs),
                    Simd::splat(y as f64),
                    rect_bounds_min_rads_w,
                    rect_bounds_min_rads_h,
                    rect_bounds_size_rads_w,
                    rect_bounds_size_rads_h,
                    src_size_s1_w,
                    src_size_s1_h,
                    out_size_s1_w,
                    out_size_s1_h,
                    &utm_proj,
                    &utm_aabb,
                );
                for (idx, src_pos) in src_x
                    .to_array()
                    .into_iter()
                    .zip(src_y.to_array())
                    .enumerate()
                {
                    out.put_pixel(
                        xs[idx] as u32,
                        y,
                        *img.get_pixel(src_pos.0 as u32, src_pos.1 as u32),
                    );
                }
            } else {
                for x in xs {
                    let (src_x, src_y) = src_pos_batched::<1>(
                        Simd::splat(x),
                        Simd::splat(y as f64),
                        rect_bounds_min_rads_w_1,
                        rect_bounds_min_rads_h_1,
                        rect_bounds_size_rads_w_1,
                        rect_bounds_size_rads_h_1,
                        src_size_s1_w_1,
                        src_size_s1_h_1,
                        out_size_s1_w_1,
                        out_size_s1_h_1,
                        &utm_proj,
                        &utm_aabb,
                    );
                    out.put_pixel(
                        x as u32,
                        y,
                        *img.get_pixel(src_x[0] as u32, src_y[0] as u32),
                    );
                }
            }
            //let color = sample_bilinear(img, src_rel.clamp(DVec2::ZERO, DVec2::ONE) * (src_size - DVec2::ONE));
        }
    }

    Ok((out, rect_bounds))
}
