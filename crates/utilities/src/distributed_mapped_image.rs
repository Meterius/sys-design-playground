    ...
    pub fn load_sub_image(
        ...
        for oy in 0..resolution.y {
            for ox in 0..resolution.x {
                ...
                // Sample from the first source whose bounds contain `world`.
...
mod tests {
    ...
    fn test_load_sub_image() {
        let im = DistributedMappedImage::from_directory("test-data/distributed_image/dir-a").unwrap();

        for (bounds, name) in [
            (DAabb2::new(dvec2(-128.0, -128.0), dvec2(128.0, 128.0)), "full"),
            ...
            let res = im.load_sub_image(bounds, uvec2(64, 64)).unwrap();
            res.save(format!("test-data/distributed_image/dir-a_{name}.png")).unwrap();
        }


        let im = DistributedMappedImage::from_directory("test-data/distributed_image/dir-b").unwrap();
        ...
        for (bounds, name) in [
            ...
            let res = im.load_sub_image(bounds, uvec2(32, 32)).unwrap();
            res.save(format!("test-data/distributed_image/dir-b_{name}.png")).unwrap();
        }
    }
}